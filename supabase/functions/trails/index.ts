import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { createClient } from "npm:@supabase/supabase-js";

const supabaseUrl = Deno.env.get("SB_URL")?.toString() || "";
const supabaseKey = Deno.env.get("SB_KEY")?.toString() || "";
const openDataKey = Deno.env.get("OPEN_DATA_KEY") || "";
const openDataUrl = Deno.env.get("OPEN_DATA_API_URL") || "";
const supabase = createClient(supabaseUrl, supabaseKey);

interface location {
  address: string;
  locality: string;
  postal_code: string;
  country: string;
  region: string;
  latitude: number;
  longitude: number;
}
interface all_trails {
  external_id: string;
  type: string | null;
  name: string | null;
  description: string | null;
  start_location: location;
  end_location: location;
  geo_line: string | null;
  url: string | null;
  license: string | null;
  images: string[] | null;
  keywords: string | null;
  publisher: string | null;
  type_of_trail: string | null;
  distance: number | null;
}

Deno.serve(async () => {
  try {
    const data = await fetchTrails();
    return new Response(JSON.stringify({ data }), { status: 200 });
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error";
    return new Response(JSON.stringify({ error: errorMessage }), {
      status: 500,
    });
  }
});

/* To invoke locally:

  1. Run `supabase start` (see: https://supabase.com/docs/reference/cli/supabase-start)
  2. Make an HTTP request:

  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/trails' \
    --header 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0' \
    --header 'Content-Type: application/json' \
    --data '{"name":"Functions"}'

*/

const fetchTrails = async () => {
  // Fetch entity IDs
  const entityIds = await fetchEntityIds();
  //  List of IDs to process
  const results: all_trails[] = [];

  for (const id of entityIds) {
    const url = `https://proxy.opendatagermany.io/api/ts/v2/kg/things/${id}`;
    const headers = {
      "Content-Type": "application/ld+json",
      "x-api-key": "ed541d4ad8f0acee0c592209e8fa2d26",
      "x-hop": "3",
    };

    try {
      const response = await fetch(url, { headers });
      const data = await response.json();
      // Extract and structure data
      const trail: all_trails = {
        external_id: id,
        type: formatType(data[0]["@type"]) || "",
        name:
          data[0]["https://schema.org/name"].find(
            (name: any) => name["@language"] === "de"
          )?.["@value"] || null,
        start_location: {
          address:
            data[0]["https://odta.io/voc/startLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/streetAddress"] || null,
          locality:
            data[0]["https://odta.io/voc/startLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/addressLocality"] || null,
          postal_code:
            data[0]["https://odta.io/voc/startLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/postalCode"] || null,
          country:
            data[0]["https://odta.io/voc/startLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/addressCountry"] || null,
          region:
            data[0]["https://odta.io/voc/startLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/addressRegion"] || null,
          latitude:
            data[0]["https://odta.io/voc/startLocation"]?.[
              "https://schema.org/geo"
            ]?.["https://schema.org/latitude"]?.["@value"] || null,
          longitude:
            data[0]["https://odta.io/voc/startLocation"]?.[
              "https://schema.org/geo"
            ]?.["https://schema.org/longitude"]?.["@value"] || null,
        },
        end_location: {
          address:
            data[0]["https://odta.io/voc/endLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/streetAddress"] || null,
          locality:
            data[0]["https://odta.io/voc/endLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/addressLocality"] || null,
          postal_code:
            data[0]["https://odta.io/voc/endLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/postalCode"] || null,
          country:
            data[0]["https://odta.io/voc/endLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/addressCountry"] || null,
          region:
            data[0]["https://odta.io/voc/endLocation"]?.[
              "https://schema.org/address"
            ]?.["https://schema.org/addressRegion"] || null,
          latitude:
            data[0]["https://odta.io/voc/endLocation"]?.[
              "https://schema.org/geo"
            ]?.["https://schema.org/latitude"]?.["@value"] || null,
          longitude:
            data[0]["https://odta.io/voc/endLocation"]?.[
              "https://schema.org/geo"
            ]?.["https://schema.org/longitude"]?.["@value"] || null,
        },
        geo_line: extractGeoLine(data[0]),
        description:
          data[0]["https://schema.org/description"]?.find(
            (desc: any) => desc["@language"] === "de"
          )?.["@value"] || null,

        url: data[0]["https://schema.org/url"]?.["@value"] || null,
        license:
          data[0]["https://schema.org/sdLicense"]?.[
            "https://schema.org/license"
          ]?.["@value"] || null,
        images: extractImages(data[0]["https://schema.org/image"]) || null,
        keywords:
          extractGermanKeywords(data[0]["https://schema.org/keywords"]) || "",
        publisher:
          data[0]["https://schema.org/sdPublisher"]?.[
            "https://schema.org/name"
          ] || null,
        type_of_trail: extractTypeOfTrail(data[0]) || null,
        distance:
          data[0]["https://schema.org/potentialAction"]?.[
            "https://schema.org/distance"
          ]?.["https://schema.org/name"] || null,
      };
      await supabase.from("trails").upsert(
        {
          external_id: trail.external_id,
          type: trail.type,
          name: trail.name,
          description: trail.description,
          start_location: trail.start_location,
          end_location: trail.end_location,
          url: trail.url,
          geo_line: trail.geo_line,
          license: trail.license,
          images: trail.images,
          keywords: trail.keywords,
          publisher: trail.publisher,
          type_of_trail: trail.type_of_trail,
          distance: trail.distance,
        },
        {
          onConflict: "external_id",
        }
      );
      results.push(trail);
    } catch (error) {
      console.error("Error fetching data for ID:", id, error);
    }
  }
  return results;
};

function formatType(type: string | string[] | undefined): string {
  if (!type) return "";

  if (Array.isArray(type)) {
    return type.join(", ");
  }

  return type.toString();
}

//Extracts German Keywords
const extractGermanKeywords = (keywords?: any[]) => {
  if (!Array.isArray(keywords)) return null;
  return keywords
    .filter(
      (keyword: { "@language": string; "@value": string }) =>
        keyword["@language"] === "de"
    )
    .map((keyword) => keyword["@value"] || "")
    .filter(Boolean)
    .join(", ");
};

//extracts Images
const extractImages = (images?: any[]) => {
  if (!images) {
    return [];
  }

  if (!Array.isArray(images)) {
    return [images["https://schema.org/contentUrl"]?.["@value"]].filter(
      Boolean
    );
  }

  return images
    .map((image: any) => image["https://schema.org/contentUrl"]?.["@value"])
    .filter(Boolean);
};

function extractTypeOfTrail(data: any): string | null {
  try {
    const typeOfTrail = data["https://odta.io/voc/typeOfTrail"];

    // Handle array case
    if (Array.isArray(typeOfTrail)) {
      return (
        typeOfTrail.find((item: any) => item["@language"] === "de")?.[
          "@value"
        ] || null
      );
    }

    // Handle single object case
    if (typeOfTrail && typeof typeOfTrail === "object") {
      return typeOfTrail["@value"] || null;
    }

    return null;
  } catch (error) {
    console.error("Error extracting type of trail:", error);
    return null;
  }
}
function extractGeoLine(data: any): string | null {
  try {
    const geoData = data["https://schema.org/geo"];

    // Handle array case
    if (Array.isArray(geoData)) {
      const geoShape = geoData.find(
        (geo) => geo["@type"] === "https://schema.org/GeoShape"
      );
      return geoShape?.["https://schema.org/line"] || null;
    }

    // Handle single object case
    if (geoData?.["@type"] === "https://schema.org/GeoShape") {
      return geoData["https://schema.org/line"] || null;
    }

    return null;
  } catch (error) {
    console.error("Error extracting geo line:", error);
    return null;
  }
}
const API_HEADERS = {
  "Content-Type": "application/ld+json",
  "x-api-key": openDataKey,
};
const QUERY_PAYLOAD = {
  "@context": {
    ds: "https://vocab.sti2.at/ds/",
    rdf: "http://www.w3.org/1999/02/22-rdf-syntax-ns#",
    rdfs: "http://www.w3.org/2000/01/rdf-schema#",
    schema: "https://schema.org/",
    sh: "http://www.w3.org/ns/shacl#",
    xsd: "http://www.w3.org/2001/XMLSchema#",
    odta: "https://odta.io/voc/",
    sq: "http://www.onlim.com/shapequery/",
    "@vocab": "http://www.onlim.com/shapequery/",
  },
  "sq:query": [
    {
      "ds:compliesWith": {
        "sq:value": "https://semantify.it/ds/nBTyKDsKX",
        "sq:datatype": "iri",
      },
      "odta:startLocation": {
        "schema:geo": {
          "sq:nearby": {
            "sq:latitude": "51.631644",
            "sq:longitude": "14.000000",
            "sq:distance": "48",
          },
        },
      },
    },
  ],
};

// Define the expected API response structure
interface ApiResponse {
  data: Array<{
    "@id": string;
    [key: string]: any;
  }>;
  metaData: {
    total: number;
    [key: string]: any;
  };
}
// Fetches entity IDs from the Open Data API
export async function fetchEntityIds() {
  let currentPage = 1;
  const pageSize = 50;
  let allIds: string[] = [];

  while (true) {
    try {
      const response = await fetch(openDataUrl + "?sortSeed=1", {
        method: "POST",
        headers: {
          ...API_HEADERS,
          "page-size": pageSize.toString(),
          page: currentPage.toString(),
        },
        body: JSON.stringify(QUERY_PAYLOAD),
      });

      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }

      // Type assert the response as ApiResponse
      const data = (await response.json()) as ApiResponse;

      // Map IDs and filter out undefined values
      const ids = data.data
        .map((item) => item["@id"].split("/entity/").pop())
        .filter((id): id is string => id !== undefined);

      allIds = allIds.concat(ids);

      const total = data.metaData.total;
      if (currentPage * pageSize >= total) {
        break;
      }
      currentPage++;
    } catch (error) {
      if (error instanceof Error) {
        console.error("Error fetching data:", error.message);
      } else {
        console.error("An unknown error occurred:", error);
      }
      break;
    }
  }
  return allIds;
}
